...
/** List daily tests, optionally filtered by class/section/date. */
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    let rows = await ctx.db.query("dailyTests").collect();
    ...
    const marks = await ctx.db.query("dailyTestMarks").collect();
...
/** Subject-wise stats for daily tests within a class/section. */
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("dailyTests", {
...
/** Remove a daily test and its marks. */
export const remove = mutation({
  args: { id: v.id("dailyTests") },
  handler: async (ctx, args) => {
    if (!(await isSchoolUser(ctx))) throw new ConvexError("Not authorized.");
    const existing = await ctx.db.get(args.id);
    if (!existing) throw new ConvexError("Test not found.");
    const marks = await ctx.db.query("dailyTestMarks").withIndex("by_test", (q) => q.eq("testId", args.id)).collect();
    for (const m of marks) await ctx.db.delete(m._id);
    await ctx.db.delete(args.id);
  },
});

/** Bulk save marks for a daily test. */
  ...
  handler: async (ctx, args) => {
    ...
    for (const entry of args.marks) {
      ...
      if (existing) {
        ...
        await ctx.db.insert("dailyTestMarks", {
          testId: args.testId,
...
/** Get marks for a specific daily test. */
export const getMarks = query({
  args: { testId: v.id("dailyTests") },
  handler: async (ctx, args) => {
    if (!(await isSchoolUser(ctx))) return [];
    const test = await ctx.db.get(args.testId);
    if (!test) return [];

    const allMarks = await ctx.db.query("dailyTestMarks")
